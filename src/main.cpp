...
int64 nChainStartTime = 1459574967;
...
const string strMessageMagic = "AGOLD Signed Message:\n";
...
bool CTransaction::IsStandard() const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        // 2014-04-19 Adriano https://agoldcointalk.org/index.php?action=profile;u=112568
        // The following address was lost during distribution with 196780608.602771 coins in it. Blocking just in case :-)
        static const CagoldcoinAddress lostWallet ("CKGK6MFmBkreG7k5sU8gDEJNVJ57QZtN3H");
        uint256 hashBlock;
        CTransaction txPrev;
        ...
        if(GetTransaction(txin.prevout.hash, txPrev, hashBlock)){  // get the vin's previous transaction
            ...
            if (ExtractDestination(txPrev.vout[txin.prevout.n].scriptPubKey, source)){  // extract the destination of the previous transaction's vout[n]
                CagoldcoinAddress addressSource(source);
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    else if(nHeight > 100 && nHeight <= 10500000)
    {
        nSubsidy = 66.6 * COIN;
    }
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal agoldcoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in AGOLD
    bool fStrictPayToScriptHash = true; // Always active in AGOLD
    ...
    // ppcoin: fees are not collected by miners as in agoldcoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "AGOLD", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Genesis fWorkL vector empty mapBlockIndex AGOLD";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = nChainStartTime + 56;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 535375 : 535375;
       ...
       // printf("block.nNonce = %u \n", block.nNonce);
		block.print();
		assert(block.hashMerkleRoot == uint256("0x08c2035418b6da5555b8d9ea785a224ea95d90a418d35394665443dc3ec82a82"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("agoldcoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("agoldcoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("agoldcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("agoldcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadagoldcoinMiner(void* parg);

static bool fGenerateagoldcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void agoldcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    RenameThread("agoldcoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateagoldcoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateagoldcoins) && !fProofOfStake)
                return;
        }
        ...
        printf("Running agoldcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadagoldcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        agoldcoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadagoldcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadagoldcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadagoldcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void Generateagoldcoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateagoldcoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateagoldcoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d agoldcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadagoldcoinMiner, pwallet))
                printf("Error: NewThread(ThreadagoldcoinMiner) failed\n");
            Sleep(10);
        }
    }
}
