    ...
    model(0)
{
    ...
    ui->sendButton->setIcon(QIcon());
     ...
     ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a AGOLD address"));
#endif
    ...
    // Coin Control
    ui->lineEditCoinControlChange->setFont(GUIUtil::agoldcoinAddressFont());
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    {
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(agoldcoinUnits::formatWithUnit(agoldcoinUnits::BTC, rcp.amount), Qt::escape(rcp.label), rcp.address));
    }
...
//    WalletModel::SendCoinsReturn sendstatus = model->sendCoins(recipients);
    ...
    case WalletModel::AmountWithFeeExceedsBalance:
        QMessageBox::warning(this, tr("Send Coins"),
            ...
            arg(agoldcoinUnits::formatWithUnit(agoldcoinUnits::BTC, sendstatus.fee)),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    // URI has to be valid
    if (GUIUtil::parseagoldcoinURI(uri, &rv))
    {
        CagoldcoinAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::setBalance(qint64 balance, qint64 stake, qint64 unconfirmedBalance, qint64 immatureBalance)
{
    ...
    ui->labelBalance->setText(agoldcoinUnits::formatWithUnit(unit, balance));
}
...
void SendCoinsDialog::updateDisplayUnit()
{
    if(model && model->getOptionsModel())
    {
        ...
        ui->labelBalance->setText(agoldcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), model->getBalance()));
    }
}
...
// Coin Control: copy label "Quantity" to clipboard
 ...
 void SendCoinsDialog::coinControlChangeChecked(int state)
 {
     ...
     {
         if (state == Qt::Checked)
             CoinControlDialog::coinControl->destChange = CagoldcoinAddress(ui->lineEditCoinControlChange->text().toStdString()).Get();
         else
 ...
 void SendCoinsDialog::coinControlChangeEdited(const QString & text)
 {
     if (model)
     {
         CoinControlDialog::coinControl->destChange = CagoldcoinAddress(text.toStdString()).Get();

         // label for the change address
        ui->labelCoinControlChangeLabel->setStyleSheet("QLabel{color:black;}");
        if (text.isEmpty())
             ui->labelCoinControlChangeLabel->setText("");
         else if (!CagoldcoinAddress(text.toStdString()).IsValid())
         {
             ui->labelCoinControlChangeLabel->setStyleSheet("QLabel{color:red;}");
             ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid AGOLD address"));
         }
         else
         {
             ...
             else
             {
                 CPubKey pubkey;
                 CKeyID keyid;
                 CagoldcoinAddress(text.toStdString()).GetKeyID(keyid);
